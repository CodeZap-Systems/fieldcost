...
const DEMO_COMPANY_ID = 8; // Demo company for testing
...
// Removed duplicate or incomplete function block
// TEST RUNNER
// ============================================================================
...
async function testProjectCRUD() {
  ...
  if (projectId) {
    try {
      const updateRes = await httpRequest('PATCH', `projects/${projectId}`, {
  ...
  if (projectId) {
    try {
      const deleteRes = await httpRequest('DELETE', `projects/${projectId}`, { user_id: DEMO_USER, company_id: DEMO_COMPANY_ID });
...
async function testInvoiceCRUD() {
  ...
  try {
    const customerRes = await httpRequest('POST', 'customers', {
      ...
      user_id: DEMO_USER,      company_id: DEMO_COMPANY_ID,    });
...
async function testDataIsolation() {
  ...
  try {
    ...
    // Live project count (use different company for live user)
    const liveRes = await httpRequest('GET', `projects?company_id=9`);
  ...
  try {
    const noUserRes = await httpRequest('GET', 'projects?company_id=invalid');
...
async function testErrorHandling() {
  ...
  // Invalid project ID
  try {
    const invalidRes = await httpRequest('GET', `projects/99999?company_id=${DEMO_COMPANY_ID}`);
