...
/**
 * Sync Items from Sage to FieldCost Database
 ...
 * 
 * Pulls items from Sage One and stores them in FieldCost database
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || authError) {
      console.warn(`⚠️  Sage items sync: Authentication failed`);
    ...
    // Get company_id from request body (required for multi-tenant isolation)
    ...
    if (!isLiveTenant(tenant)) {
      ...
      console.warn(`⚠️  Sage items sync blocked: ${reason}`);

      try {
        await logTenantAccess(
          tenant?.id || 'unknown',
          userId,
          'sage_items_sync_attempted',
          'blocked',
          reason,
          { companyId }
        );
    ...
    try {
      await logTenantAccess(
        tenant.id,
        userId,
        'sage_items_sync_started',
        'allowed',
        'Items sync initiated for live environment',
    ...
    // ============================================================================
    // Get Sage credentials (after security checks)
    // ============================================================================
    ...
    const sageItems = itemsResult.data || [];

    if (sageItems.length === 0) {
      return NextResponse.json(
        {
          success: true,
          message: "No items found in Sage to sync",
          synced: 0,
          skipped: 0,
          errors: [],
        },
    ...
    for (const sageItem of sageItems) {
      try {
        // Check if item already exists in FieldCost
        const result = await supabaseServer
          .from("items")
          .select("id")
          .eq("company_id", companyId)
          .eq("sage_item_id", sageItem.ID)
          .single();
        ...
        if (existing) {
          ...
          const { error } = await supabaseServer
            ...
            .update({
              ...
              sku: sageItem.Code,
              unit_price: sageItem.SalesPrice || 0,
              cost_price: sageItem.PurchasePrice || 0,
              quantity_on_hand: sageItem.Quantity || 0,
              unit_of_measure: sageItem.UnitOfSales || "unit",
              sage_item_id: sageItem.ID,
          ...
          if (error) {
            syncResults.errors.push(
              `Failed to update item ${sageItem.Code}: ${error.message}`
            );
            syncResults.skipped++;
          } else {
            syncResults.synced++;
          ...
          // Create new item
          const { error } = await supabaseServer.from("items").insert({
          ...
          if (error) {
            syncResults.errors.push(
              `Failed to create item ${sageItem.Code}: ${error.message}`
            );
            syncResults.skipped++;
          } else {
            syncResults.synced++;
        ...
        syncResults.errors.push(
          `Error processing item ${sageItem.Code}: ${String(error)}`
        );
        syncResults.skipped++;
      }
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Synced ${syncResults.synced} items from Sage One`,
        synced: syncResults.synced,
        skipped: syncResults.skipped,
        total: sageItems.length,
    ...
    console.error("Sage items sync error:", error);
    return NextResponse.json(
      {
        success: false,
