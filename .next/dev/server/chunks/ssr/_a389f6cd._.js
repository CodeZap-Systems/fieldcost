...
function ProjectsPage() {
    ...
    async function handleDelete(id1) {
        ...
        try {
            ...
            setProjects((prev)=>prev.filter((p)=>p.id !== id1));
        } catch  {
    ...
    async function handlePhotoUpload(projectId1, url) {
        if (!userId) return;
        setLoading(true);
        setError(null);
        try {
            ...
            const updated = await res.json();
            setProjects((prev)=>prev.map((p)=>p.id === projectId1 ? updated : p));
        } catch  {
...
//# sourceMappingURL=_a389f6cd._.js.map