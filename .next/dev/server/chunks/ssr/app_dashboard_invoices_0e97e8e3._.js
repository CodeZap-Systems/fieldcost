        ...
        async function load() {
            try {
                ...
                const [cust, itm] = await Promise.all([
                    fetch(`/api/customers?${qs.toString()}`).then((r)=>r.ok ? r.json() : []),
                    fetch(`/api/items?${qs.toString()}`).then((r)=>r.ok ? r.json() : [])
                ]);
...
//# sourceMappingURL=app_dashboard_invoices_0e97e8e3._.js.map