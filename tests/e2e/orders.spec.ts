/**
 * E2E Test Suite - Supplier Purchase Orders
 * Tests for complete supplier purchase order workflow including creation, confirmation, and delivery tracking
 * NOTE: "Purchase Orders" (POs) are for procuring materials from SUPPLIERS/VENDORS, not sales to customers
 */
...
test.describe('Supplier Purchase Order Management - E2E Tests', () => {
  let page: Page;
  ...
  test('TC201: Create supplier purchase order with single line item', async () => {
    // Navigate to purchase orders page
    await page.goto(`${BASE_URL}/dashboard/purchase-orders`);
    
    // Click add PO button
    await page.click('a:has-text("Add PO"), a:has-text("Add Purchase Order"), button:has-text("Add PO"), button:has-text("Add Purchase Order")');
    await page.waitForURL(`**/purchase-orders/add`);
    
    // Fill PO form
    await page.selectOption('select[name="vendor"], select[name="vendorId"]', { label: SAMPLE_ORDER.vendorName || 'BuildSupplies Inc' });
    await page.fill('input[name="reference"], input[name="poReference"]', SAMPLE_ORDER.reference);
    
    // Set delivery date
    if (SAMPLE_ORDER.deliveryDate) {
      await page.fill('input[name="deliveryDate"], input[name="delivery_date"], input[name="requiredBy"]', SAMPLE_ORDER.deliveryDate);
    }
    ...
    // Submit
    await page.click('button:has-text("Create Purchase Order"), button:has-text("Create PO")');
    
    // Verify
    await expect(page.locator('text=Purchase Order created successfully')).toBeVisible({ timeout: 5000 });
  });

  test('TC202: Create supplier purchase order with multiple line items', async () => {
    await page.goto(`${BASE_URL}/dashboard/purchase-orders/add`);
    
    const order = generateTestOrder();
    
    // Fill header
    await page.selectOption('select[name="vendor"], select[name="vendorId"]', { label: 'BuildSupplies Inc' });
    await page.fill('input[name="reference"], input[name="poReference"]', order.reference);
  ...
  test('TC203: Edit supplier purchase order', async () => {
    await page.goto(`${BASE_URL}/dashboard/purchase-orders`);
    ...
    // Edit reference
    const newRef = `PO-${Date.now()}`;
    await page.fill('input[name="reference"], input[name="poReference"]', newRef);
    
    // Save
    await page.click('button:has-text("Save Changes"), button:has-text("Update Purchase Order")');
